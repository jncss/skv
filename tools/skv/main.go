...
func printUsage() {
	...
	fmt.Println("  --encryption, -e <type>           Set encryption (none, aes, simplecipher) [default: none]")
	fmt.Println("  --password, -p <password>         Encryption password (required if encryption != none)")
	fmt.Println()
	...
	fmt.Println("  skv -e aes -p secret123 put secure.skv key value  # AES encryption")
	fmt.Println("  skv --encryption simplecipher --password pass putfile secure.skv data file.txt")
...
func printHelp() {
	...
	fmt.Println()
	fmt.Println("  --encryption, -e <type>")
	fmt.Println("    Set encryption algorithm for new records")
	fmt.Println("    Options: none (default), aes (AES-256), simplecipher (custom XOR)")
	fmt.Println("    Note: Requires --password option. Both keys and values are encrypted.")
	fmt.Println("          Encryption is applied BEFORE compression.")
	fmt.Println("    Examples:")
	fmt.Println("      skv -e aes -p mypassword put secure.skv key value")
	fmt.Println("      skv --encryption simplecipher --password secret putfile db.skv data file.txt")
	fmt.Println()
	fmt.Println("  --password, -p <password>")
	fmt.Println("    Password for encryption/decryption")
	fmt.Println("    Required when --encryption is not 'none'")
	fmt.Println()
