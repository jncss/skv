...
func printUsage() {
	...
	fmt.Println("  --compression, -c <type>          Set compression (none, snappy, lz4) [default: none]")
	fmt.Println()
	...
	fmt.Println("  skv -c lz4 put mydb.skv data content # Store with LZ4 compression")
	fmt.Println("  skv --compression snappy putfile mydb.skv config config.ini")
...
func printHelp() {
	...
	fmt.Println()
	fmt.Println("  --compression, -c <type>")
	fmt.Println("    Set compression algorithm for new records")
	fmt.Println("    Options: none (default), snappy (fast), lz4 (balanced)")
	fmt.Println("    Note: Only affects new writes. Existing records are read correctly")
	fmt.Println("          regardless of their compression type.")
	fmt.Println("    Examples:")
	fmt.Println("      skv -c lz4 put mydb.skv key value")
	fmt.Println("      skv --compression snappy putfile mydb.skv config file.txt")
	fmt.Println()
